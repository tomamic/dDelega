...
public class ThresholdSubject {
	...
	{
		subjects.add( SamlddHelper.createFullyQualifiedName( qualifier, localName ) );
	}
	...
	{
		return subjects.remove( SamlddHelper.createFullyQualifiedName( qualifier, localName ) );
	}
	
	/** 
	 * Adds the given subject to the threshold. The string must be a key hash made using {@link SamlddHelper}.
	 * 
	...
	/** 
	 * Removes the given subject from the threshold, if present. The subject must be a key hash obtained from {@link SamlddHelper}. 
	 * 
